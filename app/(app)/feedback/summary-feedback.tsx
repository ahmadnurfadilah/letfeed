...
export default function SummaryFeedback() {
  const summarizeFeedback = useAction(api.vector.summarizeFeedback);
  ...
  return (
    <>
      <AlertDialog>
        <AlertDialogTrigger
          className={`${buttonVariants({ variant: "brand" })} gap-2`}
        >
            ...
            <AlertDialogDescription>
              <div className="flex items-center gap-2 mb-4">
                <Button
                  ...
                  variant={activeSentiment === "all" ? "dark" : "outline"}
                  onClick={() => handleSummarize("all")}
                >
                  All Sentiment
                </Button>
                <Button
              ...
              <div className="border border-dashed p-4 rounded-md">
                {summarizing ? (
                  ...
                  <>
                    {activeSentiment === "all" && summary.all && (
                      <div className="whitespace-pre-line">{summary.all}</div>
                    )}
                    {activeSentiment === "positive" && summary.positive && (
                      <div className="whitespace-pre-line">
                        {summary.positive}
                      </div>
                    )}
                    {activeSentiment === "neutral" && summary.neutral && (
                      <div className="whitespace-pre-line">
                        {summary.neutral}
                      </div>
                    )}
                    {activeSentiment === "negative" && summary.negative && (
                      <div className="whitespace-pre-line">
                        {summary.negative}
                      </div>
                    )}
