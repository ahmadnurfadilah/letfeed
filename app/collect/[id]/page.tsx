...
export default function Page() {
  ...
  const team = useQuery(api.teams.getTeam, { id: params.id as Id<"teams"> });
  ...
  const handleSendFeedback = async () => {
    ...
    const ai = await fetch("/api/ai", { method: "POST", body: JSON.stringify({ text: description }) }).then((res) => res.json()).catch((err) => console.log(err));
    try {
      await sendFeedback({
        rate: rate,
        description: description,
        teamId: params.id as Id<"teams">,
        embedding: ai.data.vector,
